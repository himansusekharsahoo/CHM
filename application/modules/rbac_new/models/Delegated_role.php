...
$this->load->model('rbac_user');$this->load->model('rbac_role');
...
->add_column("Action", $data['button_set'], 'c_encode(delegated_role_id)', 1, 1);
	 if($export):
	 $data = $this->datatables->generate_export($export);
	 export_data($data['aaData'], $export, delegated_role, $tableHeading);
	 endif;
...
public function delete($delegated_role_id){
	 ...
	 $result=0;
            $result=$this->db->delete('delegated_role', array('delegated_role_id'=>$delegated_role_id));
return $result;
