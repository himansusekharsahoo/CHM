...
$this->load->model('library_member');$this->load->library('pagination');$this->load->library('form_validation');
...
$this->layout->navTitle='Library member edit';$data=array();
	 if($this->input->post()):
	 $data['data']=$this->input->post();
	 $config = array(
array(
...
$data=array();
	 ...
	 $this->layout->navTitle='Library member view';$result = $this->library_member->get_library_member(null, array('member_id' => $member_id),1);
	 if($result):
	 $result = current($result);
	 endif;
